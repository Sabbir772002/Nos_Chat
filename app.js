...
const pbvt = require('./models/pbvt');
...
app.get('/api/findpbvt', async (req, res) => {
    try {
        const { id1, id2 } = req.query;
        const id11 = new RegExp(`^${id1}$`, 'i'); // Case-insensitive exact match
        const id12 = new RegExp(`^${id2}$`, 'i');

        console.log("Received IDs:", id1, id2);

        // Check if the record exists in pbvt
        let pbvt1 = await pbvt.findOne({
            $or: [
                { fnd1: { $regex: id11 }, fnd2: { $regex: id12 } },
                { fnd1: { $regex: id12 }, fnd2: { $regex: id11 } },
            ],
        });
        console.log("Existing pbvt:", pbvt1);

        if (!pbvt1) {
            console.log("No existing pbvt found, fetching from external service...");

            const pbvt2 = await axios.get(`http://localhost:8000/pbvt`);
            const { e, d, n } = pbvt2.data;

            const newPbvt = new pbvt({ fnd1: id1, fnd2: id2, e, d, n });
            pbvt1 = await newPbvt.save();

            console.log('New pbvt saved:', pbvt1);
        }

        console.log("Returning pbvt:", pbvt1);
        res.status(200).json({ e: pbvt1.e, d: pbvt1.d, n: pbvt1.n });
    } catch (error) {
        console.error("Error in /api/findpbvt:", error);
